...
function Dashboard() {
    ...
    return (
        <section className=' w-full ' onClick={()=> setUpdateModal(false)}>
            {addModuleModal ?
                ...
                : null}
            {updateModal ?
                <div onClick={(e) => { setUpdateModal(false) }}  className='w-full h-full z-30 fixed left-0 top-0 py-10 flex justify-center items-center'>
            ...
            <header className='  py-1 px-10 bg-gray-50 flex items-center justify-between'>
                ...
                <div className=' flex items-center gap-2'>
                    <button onClick={(e) => { setAddModuleModal(true) }} className=' rounded-lg  hover:bg-gray-300  hover:text-black font-semibold px-3 py-1' >+ Add Module</button>
                    <div className=' relative'>
                        <button onClick={(e) => { e.stopPropagation(); setUpdateModal(!updateModal) }} className=' rounded-lg bgBlue text-white  hover:bg-gray-300 hover:text-black font-semibold px-3 py-1' >Updates</button>
                        {updateModal ?

                            <div onClick={(e) => { e.stopPropagation(); }} className=' w-[25rem] z-50 absolute right-7 bg-white border-2 rounded-md p-2 h-fit'>
                                <div className=' flex flex-col justify-center items-center text-left'>
                                    <div onClick={()=> setUpdateModal(false)} className=' bgLightBlue p-3 rounded-lg text-sm cursor-pointer'>
                                        <h1 className=' text-gray-600'>Date: 16 jul 2024</h1>
                                        <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit.</p>
                                    </div>
                                    <div onClick={()=> setUpdateModal(false)} className=' bgLightBlue p-3 rounded-lg text-sm cursor-pointer'>
                                        <h1  className=' text-gray-600'>Date: 13 jul 2024</h1>
                                        <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit.</p>
                                    </div>
                                    <div onClick={()=> setUpdateModal(false)} className=' bgLightBlue p-3 rounded-lg text-sm cursor-pointer'>
                                        <h1  className=' text-gray-600'>Date: 10 jul 2024</h1>
                                        <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit.</p>
                                    </div>
                                </div>
                            </div>
